  ...
  return (
    <div className="fixed inset-0 bg-stone-100/80 backdrop-blur-sm z-[100] flex justify-center items-center p-4">
      <div className="w-full max-w-sm flex flex-col h-[90vh] bg-white shadow-2xl rounded-2xl overflow-hidden">
        ...
        {/* 여기에 overflow-y-auto가 있어도, 카드 내부의 absolute 검색창은 튀어나올 수 있습니다 */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {items.map((item, i) => (
             <RecordCard 
                ...
                searchMovie={(v) => { setActiveIndex(i); searchMovie(v); }}
        ...
        <button disabled={isSaving} className="w-full py-5 text-[10px] uppercase tracking-[0.3em] bg-stone-900 text-white" onClick={async () => {
          setIsSaving(true);
          if (await saveLog(items, date.replace(/-/g, '.'))) { setIsDirty(false); onClose(); }
          setIsSaving(false);
