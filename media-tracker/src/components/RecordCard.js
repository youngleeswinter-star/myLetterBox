    ...
    <div className="bg-white p-5 border border-stone-100 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] rounded-lg space-y-5 transition-all focus-within:ring-2 focus-within:ring-stone-200">
      ...
      <div className="flex justify-between items-center pb-3 border-b border-stone-100">
        <div className="flex items-center gap-2.5">
          ...
          <input type="date" className="text-[10px] text-stone-700 font-medium bg-stone-100 px-2 py-1 rounded outline-none cursor-pointer hover:bg-stone-200" 
            value={item.date.split('.').map(v => v.padStart(2, '0')).join('-')} 
            onChange={(e) => updateItem({ ...item, date: e.target.value.replace(/-/g, '.') })} />
        </div>
